const API_BASE = import.meta.env.VITE_API_BASE || "https://cms.themarketingking.org/wp-json/wp/v2";

// export async function fetchPosts(categoryId) {
//   const res = await fetch(`${API_BASE}/posts?_embed&categories=${categoryId}&per_page=500`);
//   if (!res.ok) throw new Error("Failed to fetch posts");
//   return res.json();
// }

export async function fetchPosts(categoryId, page = 1, perPage = 6) {
  const res = await fetch(
    `${API_BASE}/posts?_embed&categories=${categoryId}&page=${page}&per_page=${perPage}`
  );
